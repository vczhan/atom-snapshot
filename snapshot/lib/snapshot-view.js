...
export default class SnapshotView extends View {
  ...
  setPanelSession() {
    // return ( this.session = JSON.parse(sessionStorage.getItem(key)) )
    const _id = this.getSnapshotId()
    const _session = JSON.parse(sessionStorage.getItem(_id))
    this.updatePanelSession(_session)
  }
  ...
  updateView() {
    const _session = this.getPanelSession()
    ...
    if (_session !== null && _session.list && (len = _session.list.length) > 0) {
      for ( ; item = _session.list[i++]; ) {
  ...
  addSnapshotSuccess({ name, desc }) {
    ...
    let _session = this.getPanelSession()
    ...
    if (_session === null) {
      ...
      _session.list.push(item)

    }
    sessionStorage.setItem([_id], JSON.stringify(_session))
    this.updatePanelSession(_session)
    this.updateView()
  ...
  removeSnapshotAll() {
    ...
    this.updatePanelSession(null)
    this.updateView()
  }

  selectSnapshot(index) {
    /**##################################
    # ✘ 移动文件或修改文件名没做监听更新session，
    # ✘ 所以path和title不从session取
    ##################################**/
    const activeItem = this.getActiveItem()
    const _title = activeItem.getTitle()
    const _path = activeItem.getPath()
    const _session = this.getPanelSession()
    const _text = _session.list[index].text
    const _cursor = _session.list[index].cursor
